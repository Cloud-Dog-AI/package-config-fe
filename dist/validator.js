...
export function warnIfSecrets(config) {
    ...
    for (const [key, value] of Object.entries(config)) {
        ...
        if (suspicious.some((s) => v.includes(s))) {
            ...
            console.warn(`[config] Key "${key}" may contain a secret — runtime config is public.`);
        }
    }
}
export function validateConfig(schema, raw) {
    const result = schema.safeParse(raw);
    if (!result.success) {
        throw new ConfigError(`Invalid runtime config: ${formatZodError(result.error)}`);
    }
    if (result.data && typeof result.data === "object") {
        warnIfSecrets(result.data);
    }
    return result.data;
}
